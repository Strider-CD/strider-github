...
function api_call(path, accessToken, callback, client) {
  ...
  get_oauth2(url, {}, accessToken, function (error, res) {
...
function pageinated_api_call(path, accessToken, callback, client) {
  ...
  function loop(uri, page) {
    get_oauth2(uri, {per_page: 30, page: page}, accessToken, function (error, response) {
  ...
  step(
    ...
    function finalize(err, results) {
      ...
      debug('Github results for user %s - Repos: %j Team Repos w/ admin: %j Org memberships: %j',
        user.email, _.map(repos, 'name'), _.map(team_repos, 'name'),
 ...
 */
function save_repo_deploy_keys(user_obj, gh_repo_url, privkey, pubkey, callback) {
 ...
 * @param {Object} client An alternative superagent instance to use.
 */
function add_deploy_key(gh_repo_path, pubkey, title, accessToken, callback, client) {
  client = client || superagent;
 ...
 * @param {Object} client An alternative superagent instance to use.
 */
function set_push_hook(gh_repo_path, name, url, secret, accessToken, callback, client) {
  client = client || superagent;
 ...
 */
function setup_integration(user_obj, gh_repo_id, token, callback, socket, no_ssh) {
