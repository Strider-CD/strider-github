...
function addConfiguredKeys(user, repos) {
  ...
  repos.forEach(function (repo) {
    ...
    if (repo.html_url === undefined) {
      debug(
        'addConfiguredKeys(): repo.html_url is undefined. Full repo: %j',
        repo
      );
    ...
    repo.configured =
      already_configured.indexOf(repo.html_url.toLowerCase()) != -1;
  });
}
...
module.exports.github_metadata = function (req, res) {
  ...
  var no_existing_metadata =
    req.user.github_metadata === undefined ||
    req.user.github_metadata[req.user.github.id] === undefined;
  var refresh_requested =
...
module.exports.github_webhooks_unset = function (req, res) {
  ...
  if ((url = req.param('url')) === undefined) {
    results = {
      status: 'error',
      errors: [{ message: 'you must supply a url parameter' }],
    };
    res.statusCode = 400;
  ...
  req.user.get_repo_config(url, function (err, repo) {
    if (err || !repo) {
      results = {
        status: 'error',
        errors: [{ message: 'invalid url for this user' }],
      };
      res.statusCode = 400;
    ...
    debug(
      'github.github_webhooks_unset(): unsetting hooks for user %s on repo %s',
      req.user.email,
      gh_repo_path
    );
...
module.exports.post_manual_setup = function (req, res) {
  ...
  User.findOne({ 'github_config.url': github_url.toLowerCase() }, function (
    err,
    user
  ) {
    var r;
    ...
    gh.setup_integration_manual(req, p.org, repo_url, function (
      webhook,
      deploy_key_title,
      deploy_public_key
    ) {
