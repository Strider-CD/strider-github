...
function deleteHooks(reponame, url, token, callback) {
  ...
  superagent.get(apiUrl)
    ...
    .end(function (res) {
      if (res.status > 300) return callback(res.status)
      var hooks = []
...
function getFile(filename, ref, accessToken, owner, repo, done) {
  var req = superagent.get('https://api.github.com/repos/' + owner + '/' + repo + '/contents/' + filename)
  if (ref) {
    ...
    function fetchOrgTeams(err, r, o) {
      ...
      for (var i=0; i<r.data.length; i++) {
        repos.push({
          ...
          config: {
            url: "github://" + r.data[i].clone_url.split('//')[1],
