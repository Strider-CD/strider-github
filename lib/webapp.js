...
module.exports = {
  appConfig: {
    ...
    appSecret:
      process.env.PLUGIN_GITHUB_APP_SECRET ||
      '18651128b57787a3336094e2ba1af240dfe44f6c',
    // used for web auth
  ...
  auth: function (passport) {
    ...
    if (!config.appId || !config.appSecret || !config.hostname) {
      throw new Error(
        'Github plugin misconfigured! Need `appId`, `appSecret` and `hostname`.'
      );
    }
  ...
  setupRepo: function (account, config, project, done) {
    ...
    if (!account.accessToken)
      return done(new Error('Github account not configured'));
  ...
  routes: function (app, context) {
    ...
    app.post('/hook', function (req, res) {
      ...
      if (!account.accessToken)
        return res.status(400).send('Github account not configured');
      api.createHooks(
        req.project.name,
        url,
        pconfig.secret,
    ...
    app.delete('/hook', function (req, res) {
      ...
      if (!account.accessToken)
        return res.status(400).send('Github account not configured');
      api.deleteHooks(req.project.name, url, account.accessToken, function (
        err,
        deleted
      ) {
        if (err) return res.status(500).send(err.message);
...
function validateAuth(req, accessToken, refreshToken, profile, done) {
  if (!req.user) {
    ...
    req.flash(
      'account',
      "Cannot link a github account if you aren't logged in"
    );
    return done();
  }
  ...
  if (account) {
    debug("Updating a github account that's already attached...");
    req.flash(
      'account',
      'That github account is already linked, we updated it for you. You can also <a target="_blank" href="https://github.com/logout">sign out of github</a> before you click "Add Account"'
    );
    account.config.accessToken = accessToken;
