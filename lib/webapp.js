...
module.exports = {
  ...
  routes: function (app, context) {
    app.put('/hook', function (req, res) {
      ...
      api.createHook(account.login, account.accessToken, req.project.name, function (err, created) {
        if (err) return res.send(500, err.message)
        res.send(200, created ? 'Webhook registered' : 'Webhook already existed')
      })
    })
    app.delete('/hook', function (req, res) {
      var account = req.accountConfig()
      if (!account.accessToken) return res.send(400, 'Github account not configured')
      api.deleteHook(account.login, account.accessToken, req.project.name, function (err, deleted) {
        if (err) return res.send(500, err.message)
        res.send(200, deleted ? 'Webhook removed' : 'No webhook to delete')
      })
    })
    // github should hit this endpoint
    app.post('/hook/:secret', function (req, res) {
      // trigger new build
