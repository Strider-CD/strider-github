...
module.exports = {
  ...
  listRepos: function (account, next) {
    api.getRepos(account.accessToken, account.username, function (err, repos) {
      next(err, repos)
    })
  },
  ...
  routes: function (app, context) {
    app.put('/hook', function (req, res) {
      ...
      if (!account.accessToken) return res.end(400)
      api.createHook(account.username, account.accessToken, res.repo.name, function (err, success) {
